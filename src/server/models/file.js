...
export class FileModel {
    static getDir(filter, callback) {
        ...
        const inputPath = filter.path

        const doIt = (err, files) => {
            const sWho = 'FileModel::getDir::doIt'
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                const extendedPath = path.join(inputPath, files[i])
                ...
                try {
			        const stats = fs.lstatSync(extendedPath)
