...
const jutils = require('../jutils') // jutils.js
const sharedUtils = require('../ng/shared-utils.svc.js')
...
function Objectives() {
    ...
    this.getObjectives = function (options, callback) {
        ...
        MongoClient.connect(config.mongoDbScrummerUrl, (err, db) => {
            const sWho = 'connectCallback'
            ...
            logger.info(`${sWho}(): Using collection "${config.mongoDbScrummerObjectivesCollection}"...`)
  			const collection = db.collection(config.mongoDbScrummerObjectivesCollection)
            ...
            if (options.countOnly) {
                ...
                console.log(`${sWho}(): Calling collection.find(), query = `, query, 'sort = ', sort, `skip=${iSkip}, limit=${iLimit}...`)
  				...
  				collection.find(query).sort(sort).skip(iSkip).limit(iLimit)
                    .toArray((err, items) => {
                        const sWho = 'findCallback'
    ...
    this.createObjective = function (objective, callback) {
        const sWho = 'Objectives::createObjective'
    ...
    this.updateObjective = function (objective, callback) {
        const sWho = 'Objectives::updateObjective'
        logger.info(`${sWho}(): objective = `, objective)

        const query = { _id: new ObjectID(objective._id) }
        const update = {
            project: objective.project,
            task_name: objective.task_name,
            assigned_to: objective.assigned_to,
    ...
    this.deleteObjective = function (objective, callback) {
        const sWho = 'Objectives::deleteObjective'
        logger.info(`${sWho}(): objective = `, objective)

        const filter = { _id: new ObjectID(objective._id) }
        const options = { }
    ...
    this.getFormTypes = function (options, callback) {
        ...
        MongoClient.connect(config.mongoDbUrl, (err, db) => {
            ...
            logger.info(`${sWho}(): Using collection "${config.mongoDbFormsCollection}"...`)
  			const collection = db.collection(config.mongoDbFormsCollection)

            // var query = {};
            // var query = { "form_processing_attempts.success" : { "$eq": true } };
            // var query = { "dn_denormalized" : { "$eq": true } };
            const query = { dn_denormalized: { $eq: true }, dn_company_conformed_name: { $ne: '' } }

            const field = 'form_type'
            console.log(`${sWho}(): Calling collection.distinct( "${field}", `, query, ' )...')

            collection.distinct(field, query, (err, items) => {
                if (err) {
                    logger.error(`${sWho}(): Trouble with count: "${err}"...`)
                    const rows = [
                        { error: err },
        ...
        MongoClient.connect('mongodb://localhost:27017/test', (err, db) => {
            ...
            collection.updateOne({ a: 1 }, { b: 2, a: 1 }, { upsert: true, w: 1 })
                .then((result) => {
                    ...
                    collection.findOne({ a: 1 }).then((item) => {
                        test.equal(1, item.a)
                        test.equal(2, item.b)
                        db.close()
                    })
