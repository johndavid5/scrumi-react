...
const checkDate = (diff, timeframeName, underTime, timeframe) => ((isUnderTime(diff, timeframe[timeframeName], underTime))
...
const _checkNext = (result, callback) => ((result)
    ? lessThanAMinute(result)
    : callback())

const checkNext = ([tfName, ...rest], timeframe, timestamp, now) => _checkNext(
    checkDate(getDiff(timestamp, now), tfName, breakpoints[tfName], timeframe),
    () => howLongAgo(rest, timeframe, timestamp, now),
)

const howLongAgo = (remainingTimeframe, timeframe, timestamp, now) => ((!remainingTimeframe.length)
