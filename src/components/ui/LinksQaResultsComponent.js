...
const LinksQaResultsComponent = ( props ) => {
    ...
    let sWho = "LinksQaResultsComponent"

    let linksQa = props.linksQa
    let debug = true
		...
		<div className="links-qa-results-component container-fluid" style={{paddingLeft: '1em'}}>
            <div className="run-params row">
                <div className="col-sm-8">
                    <h3 style={{whiteSpace: 'nowrap'}}>Base Path: {linksQa.basePath}</h3>
                </div>
                <div className="col-sm-4">
                    <h3 style={{whiteSpace: 'nowrap'}}>Addition Only Code: {linksQa.additionOnlyCode}</h3>
                </div>
            </div>

            <div className="run-params row">
                <div className="col-sm-4">
                    <h3 style={{whiteSpace: 'nowrap'}}>STDERR Rows: {stderr_rows.length}</h3>
                </div>
            </div>
            ...
            <div className="run-params row">
                <div className="col-sm-4">
                    <h3 style={{whiteSpace: 'nowrap'}}>STDOUT Rows: {stdout_rows.length}</h3>
                </div>
            </div>
			<table className="table">
			<tbody key="tbody">
            ...
            { ( debug == true ) ?
				<div><pre style={{fontSize: '125%', textAlign: 'left'}}>linksQa = { JSON.stringify(linksQa, null, ' ') }</pre></div> :
                ""  
    		}
            { ( debug == true ) ?
