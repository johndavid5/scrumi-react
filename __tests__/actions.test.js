...
describe('Action Creators', () => {
    ...
    describe('objectivesFilter() thunk-based action creator...', () => {
        ...
        beforeAll(() => {
            const sWho = '__tests__/actions.js: objectivesFilter(): beforeAll'
            ...
            const objectivesGefilter = objectivesFilter(le_filters)
            ...
            logajohn.debug(`${sWho}(): SHEMP: Moe, after dispatchin' objectivesFilter(), fetch.mock = `, fetch.mock)
            ourMockSnapshot = fetch.mock
            logajohn.debug(`${sWho}(): SHEMP: Moe, after dispatchin' objectivesFilter(), ourMockSnapshot = `, ourMockSnapshot)
        })
        ...
        it('should have objectives', () => {
            const sWho = '__tests__/actions.js: objectivesFilter(): should have objectives'
        ...
        it('should have timestamp', () => {
            const sWho = '__tests__/actions.js: objectivesFilter(): should have timestamp'
            logajohn.debug(`${sWho}(): SHEMP: Moe, store.getState() = `, store.getState())
            expect(store.getState().objectives.objectives_timestamp).toBeDefined()
        })

        it('should include filter items urlencoded in fetch url', () => {
            const sWho = '__tests__/actions.js: objectivesFilter(): should include filter items urlencoded in fetch url'
            logajohn.debug(`${sWho}(): SHEMP: Moe, store.getState() = `, store.getState())
    ...
    describe('objectivesIsFetching()...client-based action...(even though it uses immediately dispatched thunks...)', () => {
        beforeAll(() => {
            const sWho = '__tests__/actions.js: objectivesIsFetching():beforeAll'
            logajohn.debug(`${sWho}(): SHEMP: Moe, storeFactory = `, storeFactory)
            const bServer = true
        ...
        it('can dispatch OBJECTIVES_FETCHING true', () => {
            const sWho = '__tests__/actions.js: can dispatch OBJECTIVES_FETCHING true'
        ...
        it('can dispatch OBJECTIVES_FETCHING false', () => {
            const sWho = '__tests__/actions.js: can dispatch OBJECTIVES_FETCHING false'
