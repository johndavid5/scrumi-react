...
const sWhere = '__tests__/components/ui/ObjectivesListComponent.test.js'
...
describe('<ObjectivesListComponent /> UI Component', () => {
    it('renders timestamp if objectives.objectives_timestamp is supplied...', () => {
        const sWho = `${sWhere}: <ObjectivesListComponent /> UI Component -- renders timestamp `
           + ' if objectives.objectives_timestamp is supplied...'
        const faux_timestamp = 'MOE: No, I just left!'
        const wrapper = mount(<ObjectivesListComponent objectives={{ objectives_timestamp: faux_timestamp }} />)
        const findee = wrapper.find('#objectives-timestamp')
    ...
    it('renders filters statically...', () => {
        const sWho = `${sWhere}: <ObjectivesListComponent /> UI Component -- renders filters`

        const faux_description_filter = 'glass'
        const faux_full_name_filter = 'Charlie'
        const faux_objectives_filters = { description_filter: faux_description_filter, full_name_filter: faux_full_name_filter }

        const wrapper = mount(<ObjectivesListComponent objectives={{ objectives_filters: faux_objectives_filters }} />)

        const description_findee = wrapper.find('#static-description-filter')
        ...
        const full_name_findee = wrapper.find('#static-full-name-filter')
    ...
    it('does not render static filters if not specified...', () => {
        ...
        const faux_description_filter = ''
        const faux_full_name_filter = ''
        const faux_objectives_filters = { description_filter: faux_description_filter, full_name_filter: faux_full_name_filter }
    ...
    it('does not render timestamp if objectives.objectives_timestamp is not supplied...', () => {
        const wrapper = mount(<ObjectivesListComponent objectives={{}} />)
    ...
    it('renders table of objectives if objectives.objectives_list is populated...', () => {
        const sWho = `${sWhere}: <ObjectivesListComponent /> UI Component -- renders table of objectives`
        const wrapper = mount(<ObjectivesListComponent objectives={{ objectives_list: faux_objectives }} />)
        ...
        const num_rows = wrapper.find('#objectives-table').find('tr').length
        ...
        faux_objectives.forEach((faux_objective) => {
            ['description', 'full_name'].forEach((field) => {
                const le_id = `#${field}-${faux_objective.objective_id}`
    ...
    it('does not renders table of objectives if objectives.objectives_list is of length zero...', () => {
        const wrapper = mount(<ObjectivesListComponent objectives={{ objectives_list: [] }} />)
        expect(wrapper.find('#objectives-table').length).toBe(0)
    })

    it('shows spinning gears if objectives.objectives_fetching is true... ', () => {
        const wrapper = mount(<ObjectivesListComponent objectives={{ objectives_fetching: true }} />)
    ...
    it('does not show spinning gears if objectives.objectives_fetching is false... ', () => {
        const wrapper = mount(<ObjectivesListComponent objectives={{ objectives_fetching: false }} />)
        expect(wrapper.find('#spinning-gears').length).toBe(0)
    })
    ...
    it('click on heading invokes onObjectivesFilter() - description, preserving current filters', () => {
        const sWho = `${sWhere}: click on heading invokes onObjectivesFilter - description `

        const _onObjectivesFilter = jest.fn()

        const faux_description_filter = 'glass'
        const faux_objectives_filters = { description_filter: faux_description_filter }

        const wrapper = mount(<ObjectivesListComponent onObjectivesFilter={_onObjectivesFilter} objectives={{ objectives_filters: faux_objectives_filters, objectives_list: faux_objectives }} />)
